...
export async function getGeneInfo(symbol) {
  ...
  try {
    ...
    let searchRes = await fetch(searchUrl, { signal: AbortSignal.timeout(6000) });
    ...
    if (!idList || idList.length === 0) {
      ...
      searchRes = await fetch(searchUrl, { signal: AbortSignal.timeout(6000) });
    ...
    if (idList && idList.length > 0) {
      ...
      if (summaryRes.ok) {
        ...
        if (resultObj) {
          ...
          try {
            const linkUrl = `${NCBI_EUTILS_BASE}/elink.fcgi?dbfrom=gene&db=nuccore&id=${geneId}&term=srcdb+refseq[prop]&retmode=json`;
            const linkRes = await fetch(linkUrl, { signal: AbortSignal.timeout(4000) });
            if (linkRes.ok) {
              ...
              if (nucIds.length > 0) {
                ...
                const tSumRes = await fetch(tSumUrl, { signal: AbortSignal.timeout(4000) });
                if (tSumRes.ok) {
