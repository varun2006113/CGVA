...
export async function getClinVarVariantsForGene(geneSymbol, limit = 25) {
  ...
  try {
    ...
    if (!searchRes.ok) {
      throw new Error(`ClinVar esearch HTTP status ${searchRes.status}`);
    }
    ...
    if (!summaryRes.ok) {
      throw new Error(`ClinVar esummary HTTP status ${summaryRes.status}`);
    }
...
/**
 * Retrieve ClinVar clinical evidence specifically associated with a resolved variant.
 * Strictly queries NCBI ClinVar E-utilities for this specific variant (e.g., rsID or variation ID).
 * NO DEMO_CACHE, NO gene-wide fallback.
...
export async function getClinVarForVariant(resolvedVariant) {
  const rsid = typeof resolvedVariant === 'string'
    ? resolvedVariant
    : resolvedVariant?.rsid || resolvedVariant?.query || '';

  if (!rsid) {
    return { records: [], totalCount: 0, error: 'Missing variant identifier' };
  }

  const cleanRsid = rsid.replace(/^rs/i, '').trim();
  if (!/^\d+$/.test(cleanRsid)) {
    return { records: [], totalCount: 0, error: null };
  }

  console.log(`[clinvarService.getClinVarForVariant] Querying ClinVar for RefSNP rs${cleanRsid}`);

  try {
    // 1. Search ClinVar database for this exact rsID
    const searchUrl = `${NCBI_EUTILS_BASE}/esearch.fcgi?db=clinvar&term=rs${cleanRsid}&retmode=json`;
    const searchRes = await fetch(searchUrl, { signal: AbortSignal.timeout(6000) });
    if (!searchRes.ok) {
      return { records: [], totalCount: 0, error: `ClinVar Esearch HTTP status ${searchRes.status}` };
    }
    ...
    if (idList.length === 0) {
      console.log(`[clinvarService.getClinVarForVariant] No ClinVar records found for rs${cleanRsid}`);
      return { records: [], totalCount: 0, error: null };
    }
    ...
    if (!summaryRes.ok) {
      return { records: [], totalCount: 0, error: `ClinVar Esummary HTTP status ${summaryRes.status}` };
    }
    ...
    console.log(`[clinvarService.getClinVarForVariant] Successfully retrieved ${records.length} ClinVar records for rs${cleanRsid}`);
    ...
    console.warn(`[clinvarService.getClinVarForVariant Warning] Failed for rs${cleanRsid}:`, err.message);
    return {
      records: [],
      totalCount: 0,
      error: 'ClinVar data is temporarily unavailable.'
